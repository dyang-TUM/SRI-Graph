...
class Canvas(QWidget):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.selected_shape_hist = []
        self.line_list = []

        self.line_obj_list = []
        self.selected_shape_pair = []
        ...
        if ev.button() == Qt.LeftButton:
            if self.drawing():
                ...
                if selection is not None:
                    self.selected_shape_hist.append(selection)
    ...
    def select_predicate_shape(self, shapes):
        self.selected_shape_pair = shapes
        for shape in shapes:
            shape.selected = True
            self.selected_shape = shape
            self.set_hiding()
    ...
    def de_select_predicate_shape(self):
        if self.selected_shape_pair is not None:
            for shape in self.selected_shape_pair:
                self.selected_shape = shape
                self.de_select_shape()
                self.update()
    ...
    def select_shape_point(self, point):
        ...
        self.de_select_shape()
        self.de_select_predicate_shape()
        self.highlight_line = None
        ...
        if self.draw_line:
            for i in self.line_list:
                ...
                if self.highlight_line is not None:
                    if len(self.highlight_line):
                        if self.highlight_line[0] == i[2] and self.highlight_line[1] == i[0]:
                            p.setPen(QColor(0, 0, 0))
                p.drawLine(i[0], i[1], i[2], i[3])
                p.setPen(QColor(0,0,0))
